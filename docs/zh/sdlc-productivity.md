...
观察 agy 如何读取您的工作区 —— 它会索引 git 仓库，读取文件内容，并结合上下文进行响应。这是**自动**的：无需配置，也无需事先编写提示词。

!!! tip ".agents/ 文件夹"
    在您的首次会话之后，检查 `.agents/` —— agy 创建了跟踪您工作区的项目配置文件。这就是它在未来的运行中知道要索引什么内容的方式。
...
使用 `/permissions` 斜杠命令来查看或更改当前级别。你也可以在 `settings.json` 中设置细粒度的规则：
...
- **`.agents/rules.md`**（或 `.agents/rules/*.md`）— 作为系统提示词指令注入的项目级规则。将其用于硬性要求，例如“永远不要删除迁移文件”或“始终使用 TypeScript 严格模式”。
- **`.gemini/`** — 为了与 Gemini CLI 兼容，agy 会同时读取 `.gemini/` 目录和 `.agents/` 目录。
...
| `?` | 获取帮助并列出所有斜杠命令 |
| `alt+enter` / `ctrl+j` / `shift+enter` | 在你的提示词中插入换行符（多行输入） |
...
Antigravity CLI 的插件系统有一个独特之处：它可以**导入您已经在 Gemini CLI 中安装的插件** — 无需重新安装或重新配置。您现有的投资将得以延续。
...
!!! warning "自定义主题会被静默丢弃"
    自定义主题组件无法 1:1 迁移到 agy 的模型，在导入过程中会被跳过且不报错。如果主题对您的工作流很重要，请在导入后检查已激活的插件。
