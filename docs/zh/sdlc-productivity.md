...
> **你的第一次真正的 Antigravity CLI 会话。** 本模块涵盖了核心的日常工作流——理解代码、重构、生成测试和审查更改——以及如何使用插件扩展 CLI 以适应你团队的工具链。
...
观察 agy 是如何读取您的工作区的 —— 它会索引 git 仓库，读取文件内容，并结合上下文进行响应。这是**自动的**：无需配置，也无需预先编写提示词。

!!! tip ".agents/ 文件夹"
    在您的首次会话之后，检查 `.agents/` 文件夹 —— agy 创建了用于跟踪您工作区的项目配置文件。这就是它在未来运行时知道该索引什么内容的方式。
...
!!! tip "使用 -i 启动预置会话"
    `agy -i "<task>"`（`--prompt-interactive` 的简写）以一个提示词开始，但保持交互状态。非常适合定向探索 —— 你设定方向，然后通过跟进进行引导。
...
agy 拥有一个**3 级权限模型**，用于控制其处理工具审批的方式：
...
## 1.5 — 使用 AGENTS.md 的项目上下文 <span class="duration-badge">5 min</span>
...
| `/permissions` | 设置代理自主级别（`request-review`、`always-proceed`、`strict`） |
| `/model` | 选择推理模型（跨会话持久化） |
...
> 📖 完整按键绑定参考：[使用 Antigravity CLI](https://antigravity.google/docs/cli-using)
...
Antigravity CLI 的插件系统有一个独特之处：它可以**导入您已经在 Gemini CLI 中安装的插件** — 无需重新安装或重新配置。您现有的投入得以保留。
...
显示每个插件的名称、来源、导入日期和组件（技能、命令、MCP 服务器、代理）。
...
!!! warning "自定义主题会被静默丢弃"
    自定义主题组件无法 1:1 迁移到 agy 的模型中，在导入过程中会被跳过且不会报错。如果主题对您的工作流很重要，请在导入后检查您已激活的插件。
