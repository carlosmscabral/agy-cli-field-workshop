...
agy 会扫描您本地的 Gemini CLI 安装，发现所有已安装的插件，并将它们的组件（技能、命令、MCP 服务器、代理）暂存到位于 `~/.gemini/antigravity-cli/` 的 agy 配置中。
...
```bash
~/.gemini/antigravity-cli/plugins/<名称>/
├── plugin.json
├── mcp_config.json
├── hooks.json
├── skills/
