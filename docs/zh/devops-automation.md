...
## 3.0 — 打印模式：非交互式核心 <span class="duration-badge">5 min</span>

`--print`（简写：`-p`）是 agy 的无头模式。它运行单个提示词，打印响应，然后退出。没有交互式会话，也没有提示。
...
输出会发送到标准输出（stdout）——你可以通过管道传输、重定向或存储它。
...
默认情况下，agy 会索引包含当前目录的 git 仓库。`--add-dir` 将其扩展到其他目录。
...
沙盒通过 `settings.json` 进行配置（可以是项目级别的 `.agents/settings.json` 或用户级别的 `~/.gemini/antigravity-cli/settings.json`）：
