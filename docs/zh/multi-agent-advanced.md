...
> **agy 超越聊天助手之处。** 本模块涵盖了使 agy-cli 区别于其他所有 AI 编码工具的特性：并行子代理、使用 `/btw` 进行任务中引导、后台调度以及会话恢复。

---

## 4.0 — agy 代理模型 <span class="duration-badge">5 min</span>

agy-cli 可以派生 **子代理**（subagents）—— 并行运行的隔离任务运行器，每个都拥有自己的工作区上下文。与在多个终端选项卡中运行独立的 agy 会话不同，子代理是协同工作的：它们可以共享工作区、在隔离的分支上工作，或者在克隆的副本上操作。

三种工作区模式：

| 模式 | 含义 | 适用场景 |
| :-- | :-- | :-- |
| `inherit` | 子代理共享同一个工作区 | 附加任务 —— 预计不会发生冲突 |
| `branch` | 子代理获取隔离的克隆副本 | 对相同文件进行并行修改 |
| `share` | Git 工作树 —— 隔离的分支，共享的仓库 | 真正的并行开发 |

### 切换模型

使用 `/model` 可以在会话中途切换活动模型 —— 当你需要对特定任务进行更深度的推理时非常有用：
...
---
## 4.1 — 生成子代理 <span class="duration-badge">15 分钟</span>
...
agy 将生成一个子代理，报告其 ID，并继续您的主会话。子代理独立工作。
...
!!! tip "惊艳时刻"
    三个专门的代理在您的代码库上并行运行，每个代理都具有完整的上下文，每个代理都产生独立的发现。正是这种模式使 agy 与基于聊天的助手产生了质的区别。
...
对抗性审查模式对于安全敏感的更改、基础设施修改或任何仅仅说“看起来不错”还不够的 PR 来说特别强大。

---
## 4.2 — /btw：任务中途引导 <span class="duration-badge">10 min</span>

> **模式：无中断引导** — 在不停止运行中任务的情况下注入上下文。
...
agy 会在不停止的情况下将你的备注整合到正在进行的任务中。这就像在冲刺（sprint）中途给开发人员留下一张便利贴——他们看到后就会进行调整。
...
!!! info "与中断的对比"
    如果没有 `/btw`，引导一个长时间运行的任务意味着取消它，调整你的提示词，然后重新开始——这会丢失所有进度。`/btw` 让你能够纠正方向而无需付出这种代价。

---
## 4.3 — 后台执行与调度 <span class="duration-badge">10 min</span>
...
agy 会在不阻塞终端的情况下运行审计。完成后，您会收到包含结果的通知。
...
!!! warning "调度是会话持久化的"
    只要 agy 正在运行，调度任务就会在会话之间保持持久化。检查 `/tasks` 以查看和管理调度任务。

---
## 4.4 — 会话恢复 <span class="duration-badge">5 min</span>

> **模式：长时间运行的工作** — 准确地从你离开的地方继续。
> 📖 完整参考：[Using Antigravity CLI](https://www.antigravity.google/docs/cli-using)
...
这就是多代理事件分类排查——两个并行调查，且可在运行中途进行引导。
...
→ **[速查表](cheatsheet.md)** — 将所有四个模块的每个命令集中在一处
