...
> **这是 agy 超越聊天助手的地方。** 本模块涵盖了使 agy-cli 区别于其他所有 AI 编码工具的功能：并行子代理、使用 `/btw` 进行任务中引导、后台调度以及会话恢复。

---
## 4.0 — agy 代理模型 <span class="duration-badge">5 分钟</span>

agy-cli 可以生成**子代理**——并行运行的独立任务运行器，每个子代理都有自己的工作区上下文。与在多个终端选项卡中运行独立的 agy 会话不同，子代理是协同工作的：它们可以共享工作区、在独立的分支上工作，或者在克隆的副本上操作。

三种工作区模式：

| 模式 | 含义 | 适用场景 |
|---|---|---|
| `inherit` | 子代理共享相同的工作区 | 附加型任务——预计不会发生冲突 |
| `branch` | 子代理获得一个独立的克隆副本 | 对相同文件进行并行修改 |
| `share` | git 工作树——独立分支，共享仓库 | 真正的并行开发 |

### 切换模型

在会话期间使用 `/model` 切换活动模型——当你需要对特定任务进行更深度的推理时，这非常有用：
...
对抗性审查模式特别适用于安全敏感的更改、基础设施修改，或任何仅仅“看起来不错（LGTM）”还不够的 PR。
...
agy 会在不停止的情况下将您的注释整合到正在进行的任务中。这就像在冲刺阶段（sprint）中途给开发人员留下一张便利贴——他们看到后会进行调整。
...
> **模式：异步 agy** — 启动长时间运行的任务，并在完成时收到通知。
...
agy 会在不阻塞终端的情况下运行审计。完成时，您会收到包含结果的通知。
...
!!! warning "调度具有会话持久性"
    只要 agy 正在运行，调度任务就会跨会话持久存在。请检查 `/tasks` 以查看和管理调度任务。
...
这是多代理事件分类排查——两个并行的调查，且可在运行中途进行引导。
