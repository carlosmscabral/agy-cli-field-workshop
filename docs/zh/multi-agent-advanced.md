...
> **这是 agy 超越聊天助手的地方。** 本模块涵盖了将 agy-cli 与其他所有 AI 编码工具区分开来的功能：并行子代理、使用 `/btw` 进行任务中途引导、后台调度以及会话恢复。

---

## 4.0 — agy 代理模型 <span class="duration-badge">5 分钟</span>

agy-cli 可以生成**子代理**（subagents） — 这些是并行运行的隔离任务运行器，每个都有自己的工作区上下文。与在多个终端选项卡中运行独立的 agy 会话不同，子代理是协同工作的：它们可以共享工作区、在隔离的分支上工作，或者在克隆的副本上操作。
...
| `branch` | 子代理获取一个隔离的克隆 | 对相同文件进行并行修改 |
| `share` | git 工作树 — 隔离的分支，共享的仓库 | 真正的并行开发 |

### 切换模型

使用 `/model` 在会话中途切换活动模型 — 当您需要对特定任务进行更深入的推理时，这非常有用：
...
对抗性审查模式对于安全敏感的更改、基础设施修改或任何仅仅说“看起来不错 (looks good to me)”还不够的 PR 来说尤其强大。

---

## 4.2 — /btw：任务中途引导 <span class="duration-badge">10 min</span>

> **模式：不中断引导** — 在不停止运行中任务的情况下注入上下文。

`/btw` 是 agy 最具特色的功能之一。当 agy 正在执行任务时，你可以向它发送消息而无需取消当前操作。
...
agy 会在不停止的情况下将你的备注整合到正在进行的任务中。这就像在冲刺（sprint）中途给开发人员留下一张便签——他们看到后就会进行调整。
...
!!! info "与中断的对比"
    如果没有 `/btw`，引导一个长时间运行的任务意味着取消它、调整你的提示词并重新启动——从而丢失所有进度。`/btw` 让你无需付出这种代价即可纠正方向。
...
当您退出 agy 会话时，agy 会打印出恢复该会话的确切命令：
...
→ **[速查表](cheatsheet.md)** — 集中汇总所有四个模块的全部命令

→ **[参考：DevOps 模式](devops-automation.md)** — `--print` 流水线、CI/CD、沙盒深入探讨
