...
> **这是 agy 超越普通聊天助手的地方。** 本模块涵盖了使 agy-cli 区别于所有其他 AI 编码工具的特性：并行的子代理、使用 `/btw` 进行任务中途引导、后台调度以及会话恢复。

---

## 4.0 — agy 代理模型 <span class="duration-badge">5 分钟</span>

agy-cli 可以生成**子代理**——并行运行的隔离任务执行器，每个子代理都有自己的工作区上下文。与运行多个带有独立 agy 会话的终端选项卡不同，子代理是协同工作的：它们可以共享工作区、在隔离的分支上工作，或者在克隆的副本上操作。
...
| `branch` | 子代理获得一个隔离的克隆 | 对相同文件进行并行修改 |
| `share` | git 工作树——隔离的分支，共享的仓库 | 真正的并行开发 |

### 切换模型

在会话中途使用 `/model` 切换活动模型——当你希望针对特定任务进行更深度的推理时非常有用：
...
agy 将派生一个子代理，报告其 ID，并继续您的主会话。子代理独立工作。
...
- **内置类型：** `research`（网络搜索）、`browser`（浏览器自动化）、`self`（通用）
...
!!! tip "惊艳时刻"
    三个专门的代理在您的代码库上并行运行，每个代理都具有完整的上下文，每个代理都产生独立的发现。正是这种模式使 agy 在本质上不同于基于聊天的助手。
...
对抗性审查模式对于安全敏感的更改、基础设施修改或任何仅仅“看起来不错 (looks good to me)”还不够的 PR 特别强大。
...
`/btw` 是 agy 最具特色的功能之一。当 agy 处于任务中途时，您可以向它发送消息，而无需取消当前操作。
...
agy 会在不停止的情况下将您的备注整合到正在进行的任务中。这就像在冲刺（sprint）中途给开发人员留下一张便利贴——他们看到后会进行调整。
...
!!! info "与中断的对比"
    如果没有 `/btw`，引导一个长时间运行的任务意味着取消它、调整您的提示词并重新启动——从而丢失所有进度。`/btw` 让您无需付出这种代价即可纠正方向。
...
agy 会在不阻塞您的终端的情况下运行审计。完成时，您会收到包含结果的通知。

### 调度任务

agy 支持 cron 风格的调度，用于定期分析：
...
!!! warning "调度是会话持久的"
    只要 agy 正在运行，调度任务就会跨会话持久存在。查看 `/tasks` 以查看和管理调度任务。

---

## 4.4 — 会话恢复 <span class="duration-badge">5 分钟</span>

> **模式：长期运行的工作** — 准确地从您上次离开的地方继续。
...
→ **[速查表](cheatsheet.md)** — 一处汇总所有四个模块的全部命令
