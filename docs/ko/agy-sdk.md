# 모듈 3: SDK를 사용한 AGY 에이전트 구축

<div class="module-header" markdown>
**소요 시간:** 약 90분  
**목표:** `google-antigravity` Python 라이브러리를 사용하여 처음부터 프로덕션 수준의 AGY 에이전트 구축 — 도구, 훅, 정책, 세션 상태, 다중 에이전트 오케스트레이션 및 구조화된 출력.  
**실습:** [실습 10: 첫 번째 에이전트](exercises/ex10_first_agent.md) · [실습 11: 다중 에이전트 파이프라인](exercises/ex11_multi_agent_pipeline.md)
</div>
...
CLI는 **범용 어시스턴트**입니다. SDK로 구축한 에이전트는 **전문가**입니다. 즉, 좁은 범위의 작업, 도메인 특화 도구, 세심하게 엔지니어링된 시스템 프롬프트를 가지며, 팀 전체가 호출할 수 있는 서비스로 배포될 수 있습니다.
...
> **API 키 vs Vertex AI:** 빠른 로컬 개발을 위해서는 `LocalAgentConfig`에서 `api_key="AIza..."`를 사용하세요. GCP의 프로덕션 환경에서는 `gcloud auth application-default login`으로 인증하세요. 라이브러리가 ADC를 자동으로 인식합니다.

---

## 3.2 — 핵심 기본 요소: 에이전트, Config, 도구 <span class="duration-badge">20분</span>

`google-antigravity` SDK에는 세 가지 구성 요소가 있습니다: `Agent`, `LocalAgentConfig`, 그리고 도구(일반 Python 함수)입니다. 이것들을 배우면 무엇이든 빌드할 수 있습니다.

### 도구

도구는 **일반 Python 함수**입니다. 래퍼 클래스나 데코레이터가 없습니다. 에이전트는 독스트링(docstring)을 기반으로 언제 호출할지 결정합니다. 이것이 인터페이스 계약의 전부입니다.
...
> - 독스트링은 도구의 스키마입니다 — 모델은 이를 읽고 언제 어떻게 도구를 호출할지 결정합니다. 사람이 아닌 모델을 위해 작성하세요.
...
> **`async with Agent(config) as agent:`** — 항상 컨텍스트 관리자를 사용하세요. 이것은
...
> `gemini-1.5-flash`, `gemini-1.5-pro`는 **절대 사용하지 마세요**. 더 이상 사용되지 않습니다(Deprecated).

### 스킬

스킬은 런타임에 로드되어 도메인 지식을 주입하는 `SKILL.md` 파일입니다. 시스템 프롬프트를 간결하게 유지하세요 — 파일에서 전문 지식을 로드하세요:
...
스킬은 `LocalAgentConfig(skills_paths=["/path/to/skills/"])`를 통해 기본적으로 로드될 수도 있습니다 — SDK가 `SKILL.md` 파일을 자동으로 검색합니다.

---

## 3.3 — 정책 및 안전 <span class="duration-badge">10분</span>

**정책은 가장 먼저 설정해야 하는 항목입니다** — 정책은 사람의 승인 없이 에이전트가 수행할 수 있는 작업을 제어합니다. 모든 `LocalAgentConfig`에는 `policies=` 목록이 필요합니다:
...
## 3.4 — 훅: 관측성 및 제어 <span class="duration-badge">10 min</span>
...
`google-antigravity`에는 `SequentialAgent` 또는 `ParallelAgent` 클래스가 없습니다. 다중 에이전트는 두 가지 방식으로 수행됩니다: **모델 주도형(model-driven)** (에이전트가 서브에이전트를 생성하도록 함) 또는 **Python 주도형(Python-driven)** (`Agent` 인스턴스를 직접 오케스트레이션함).
...
> **병렬 처리를 사용해야 하는 경우:** N개의 독립적인 분석이 있을 때마다 사용합니다. 이는 순차적으로 실행할 때와 비교하여 실제 소요 시간(wall-clock time)을 60~80% 단축합니다.
...
유지보수성을 위해 에이전트 프로젝트 구조를 다음과 같이 구성하세요:
...
**소요 시간:** 45분  
**빌드:** 파일을 읽고, 문제를 식별하며, 구조화된 리뷰 보고서를 생성하는 **코드 리뷰 에이전트(Code Review Agent)**.

**구현할 내용:**

1. 3개의 도구 정의: `read_file`, `list_directory`, `record_finding` (`ToolContext` 포함)
...
**소요 시간:** 45분  
**빌드:** **작성 후 감사 파이프라인(Write-then-Audit Pipeline)** — 테크니컬 라이터 에이전트가 문서를 생성한 다음, 컴플라이언스 분석가가 GDPR 위반 사항이 없는지 감사합니다.

**구현할 내용:**

1. `skills_paths`를 통해 GDPR SKILL.md가 로드된 `technical_writer` 에이전트 빌드
2. `response_schema=ComplianceReport`를 사용하는 `compliance_analyst` 에이전트 빌드
...
| 기본 요소 | 기능 | 사용 시기 |
| :-- | :-- | :-- |
| `Agent` | 도구, 훅, 정책을 갖춘 단일 LLM 에이전트 | 핵심 요소 — 모든 에이전트가 여기서 시작됨 |
| `LocalAgentConfig` | 모든 설정을 한 곳에서 관리 (모델, 도구, 정책, 훅) | 항상 |
| `tools=[fn]` | 일반 Python 호출 가능 객체(callable), 독스트링(docstring)이 스키마 역할을 함 | 모든 외부 작업 |
...
| `on_file_change(path, fn)` | 파일 시스템 이벤트 발생 시 에이전트 트리거 | 실시간 코드 감시자(watcher) |
...
→ **[모듈 4: 멀티 에이전트 및 고급 패턴](multi-agent-advanced.md)**으로 계속 진행하기

→ 참고: **[치트시트](cheatsheet.md)** — 모든 명령어를 한 곳에 정리
