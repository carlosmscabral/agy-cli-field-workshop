...
> **agy가 채팅 어시스턴트를 뛰어넘는 부분입니다.** 이 모듈에서는 agy-cli를 다른 모든 AI 코딩 도구와 차별화하는 기능들을 다룹니다. 병렬 서브에이전트, `/btw`를 사용한 작업 중 스티어링, 백그라운드 스케줄링, 세션 재개 기능이 포함됩니다.
...
- **내장 유형:** `research` (웹 조사), `browser` (브라우저 자동화), `self` (범용)
...
!!! tip "놀라운 순간"
    세 개의 특화된 에이전트가 코드베이스에서 병렬로 실행되며, 각각 전체 컨텍스트를 가지고 독립적인 결과를 생성합니다. 이것이 agy를 채팅 기반 어시스턴트와 질적으로 다르게 만드는 패턴입니다.
...
agy는 작업을 멈추지 않고 진행 중인 작업에 여러분의 메모를 통합합니다. 이는 스프린트 중간에 개발자에게 포스트잇을 남기는 것과 같습니다. 개발자는 이를 보고 조정합니다.
...
!!! info "중단과의 차이점"
    `/btw`가 없다면, 장기 실행 작업을 스티어링하는 것은 작업을 취소하고, 프롬프트를 조정한 다음, 다시 시작하는 것을 의미하며, 이로 인해 모든 진행 상황을 잃게 됩니다. `/btw`를 사용하면 이러한 비용 없이 경로를 수정할 수 있습니다.

---
## 4.3 — 백그라운드 실행 및 스케줄링 <span class="duration-badge">10 min</span>
...
터미널에서 이 명령어를 직접 사용하여 바로 다시 돌아갈 수 있습니다.

### 사용 사례: 며칠에 걸친 기능 개발 작업
...
agy는 작성된 코드, 내려진 결정, 그리고 해결되지 않은 질문을 포함한 전체 컨텍스트를 유지합니다.
...
→ **[치트시트](cheatsheet.md)** — 4개 모듈의 모든 명령어를 한곳에 정리했습니다
