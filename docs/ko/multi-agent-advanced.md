# 모듈 4: 멀티 에이전트 및 고급 기능 <span class="duration-badge">45 min</span>
...
agy-cli는 병렬로 작동하며 각각 고유한 작업 공간 컨텍스트를 갖는 격리된 작업 실행기인 **서브에이전트**를 생성할 수 있습니다. 별도의 agy 세션으로 여러 터미널 탭을 실행하는 것과 달리 서브에이전트는 서로 조정됩니다. 작업 공간을 공유하거나, 격리된 브랜치에서 작업하거나, 복제된 복사본에서 작동할 수 있습니다.
...
| `branch` | 서브에이전트가 격리된 복제본을 얻음 | 동일한 파일에 대한 병렬 변경 |
| `share` | git 워크트리 — 격리된 브랜치, 공유 저장소 | 진정한 병렬 개발 |

### 모델 전환

세션 중간에 활성 모델을 전환하려면 `/model`을 사용하세요. 특정 작업에 대해 더 강력한 추론이 필요할 때 유용합니다.

```
/model
```

사용 가능한 옵션(Gemini 3.5 Flash, Gemini 3.1 Pro, Claude Sonnet 4.6 등)을 보여주는 모델 선택기가 열립니다.
...
> **패턴: 병렬 실행** — 여러 에이전트를 동시에 작업하도록 파견합니다.
...
- **최대 깊이:** 10 (서브에이전트는 자체 서브에이전트를 최대 10단계까지 생성할 수 있습니다)
- **기본 제공 유형:** `research` (웹 검색), `browser` (브라우저 자동화), `self` (범용)
...
3개의 독립적인 분석이 동시에 실행되는 것을 지켜보세요. 분석이 완료되면 agy가 결과를 종합합니다.

!!! tip "놀라운 순간(The Wow Moment)"
    코드베이스에서 3개의 특화된 에이전트가 병렬로 실행되며, 각각 전체 컨텍스트를 가지고 독립적인 결과를 생성합니다. 이것이 agy를 채팅 기반 어시스턴트와 질적으로 다르게 만드는 패턴입니다.
...
적대적 검토자 패턴은 보안에 민감한 변경 사항, 인프라 수정 또는 "좋아 보이네요(looks good to me)" 정도로는 충분하지 않은 모든 PR에 특히 강력합니다.
...
agy는 멈추지 않고 진행 중인 작업에 여러분의 메모를 반영합니다. 이는 스프린트 중간에 개발자에게 포스트잇을 남기는 것과 같습니다. 개발자는 이를 보고 조정합니다.
...
!!! info "중단과의 차이점"
    `/btw`가 없다면 장기 실행 작업을 스티어링하는 것은 작업을 취소하고 프롬프트를 조정한 다음 다시 시작하는 것을 의미하며, 모든 진행 상황을 잃게 됩니다. `/btw`를 사용하면 이러한 비용 없이 경로를 수정할 수 있습니다.

---
## 4.3 — 백그라운드 실행 및 예약 <span class="duration-badge">10분</span>
...
agy는 반복적인 분석을 위해 cron 스타일의 예약을 지원합니다:
...
!!! warning "예약은 세션 간 유지됩니다"
    예약된 작업은 agy가 실행되는 동안 세션 간에 유지됩니다. 예약된 작업을 확인하고 관리하려면 `/tasks`를 확인하세요.

---
## 4.4 — 세션 재개 <span class="duration-badge">5 min</span>
...
최근 대화 목록을 보여주는 세션 선택기가 열립니다. 재개할 세션을 선택하세요.
...
이것은 다중 에이전트 사고 분류입니다. 두 개의 병렬 조사가 진행되며, 실행 도중에도 방향을 조정할 수 있습니다.
